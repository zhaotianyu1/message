 ...
 * 控制器抽象类
 */
public abstract class RcsChatbotVideoPlayerController
        extends FrameLayout implements View.OnTouchListener {

    private Context mContext;
    protected RcsChatbotVideoPlayerInterface mNiceVideoPlayer;
    ...
    public RcsChatbotVideoPlayerController(Context context) {
        super(context);
        mContext = context;
     ...
     *                  <ul>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_IDLE}</li>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_PREPARING}</li>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_PREPARED}</li>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_PLAYING}</li>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_PAUSED}</li>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_ERROR}</li>
     *                  <li>{@link RcsChatbotVideoPlayer#STATE_COMPLETED}</li>
     *                  </ul>
     */
     ...
     *                 <ul>
     *                 <li>{@link RcsChatbotVideoPlayer#MODE_NORMAL}</li>
     *                 <li>{@link RcsChatbotVideoPlayer#MODE_FULL_SCREEN}</li>
     *                 <li>{@link RcsChatbotVideoPlayer#MODE_TINY_WINDOW}</li>
     *                 </ul>
     */
    ...
    protected void startUpdateProgressTimer() {
        ...
        if (mUpdateProgressTimerTask == null) {
            mUpdateProgressTimerTask = new TimerTask() {
                @Override
                public void run() {
                    RcsChatbotVideoPlayerController.this.post(new Runnable() {
                        @Override
                        public void run() {
                            updateProgress();
                        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        if (mDownX < getWidth() * 0.5f) {
                            ...
                            mGestureDownBrightness = RcsChatbotVideoUtil.scanForActivity(mContext)
                ...
                if (mNeedChangeBrightness) {
                    ...
                    WindowManager.LayoutParams params = RcsChatbotVideoUtil.scanForActivity(mContext)
                            .getWindow().getAttributes();
                    params.screenBrightness = newBrightnessPercentage;
                    RcsChatbotVideoUtil.scanForActivity(mContext).getWindow().setAttributes(params);
    ...
    protected abstract void videoAboutLink(String urlLink, String urlName);
}
