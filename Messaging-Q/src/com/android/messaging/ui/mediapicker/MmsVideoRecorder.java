...
class MmsVideoRecorder extends MediaRecorder {
    ...
    // juphoon 视频时长限制
    private static final long MIN_DURATION_LIMIT_SECONDS_RCS = 90;
    ...
    public MmsVideoRecorder(final Camera camera, final int cameraIndex, final int orientation,
        ...
        long durationLimit = MIN_DURATION_LIMIT_SECONDS;
        // juphoon
        boolean isRcs = RcsMmsUtils.getIsRcsMode();
        if (isRcs) {
            mCamcorderProfile =
                    CamcorderProfile.get(cameraIndex, CamcorderProfile.QUALITY_LOW);
            durationLimit = MIN_DURATION_LIMIT_SECONDS_RCS;
            maxMessageSize = RcsCallWrapper.rcsGetFileTransMaxSize() * 1024;
        } else {
            mCamcorderProfile =
                    CamcorderProfile.get(cameraIndex, CamcorderProfile.QUALITY_LOW);
        }
       ...
       // setOutputFormat(mCamcorderProfile.fileFormat);
		...
		//juphoon
        if (isRcs) {
            setMaxDuration((int)durationLimit * 1000);
        }
    }
