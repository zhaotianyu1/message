...
public class MediaPicker extends Fragment implements DraftMessageSubscriptionDataProvider {
    ...
    /**
     * juphoon 下层chooser的点击事件回调给上层
     */
    public interface ExternalViewClick {
        void onItemClick(String itemTag);
    }

    /**
     * juphoon 控制下面chooser的接口，从上层界面调用可以控制下面的chooser
     */
    public interface ExternalViewController {
        void setOnClickListener(ExternalViewClick click);
        void setSelectState(String tag, boolean select);
        void setEnableState(String tag, boolean enable);
        void setMoreChooserMessage(String tag, String msg);
    }
    ...
    public interface MediaPickerListener {
        ...
        /**
         *  juphoon
         * @param externalViewController
         */
        void ExternalViewController(ExternalViewController externalViewController);
    }
    ...
    public static final int MEDA_TYPE_MORE         = 0x0040; //juphoon
    ...
    public MediaPicker(final Context context) {
        ...
        mChoosers = new MediaChooser[] {
            ...
            //new LocationChooser(this,context)
            // juphoon
//            new RcsMoreChooser(this, context)
        };
    ...
    public View onCreateView(
        ...
        mTabStrip.setBackgroundResource(R.color.biaoti);
        for (final MediaChooser chooser : mChoosers) {
            ...
            Log.i("nbv","tabButton----:"+chooser.getIconResource());
            if(chooser.getIconResource() == R.drawable.act_photo){
                Log.i("nbv","tabButton----:true");
        ...
        final boolean isTouchExplorationEnabled = AccessibilityUtil.isTouchExplorationEnabled(
                getActivity());
        Log.i("vvv","isTouchExplorationEnabled:"+isTouchExplorationEnabled);
...
//        }
    ...
    void setSupportedMediaTypes(final int mediaTypes) {
        ...
        for (final MediaChooser chooser : mChoosers) {
            ...
            final ImageButton tabButton = chooser.getTabButton();
            if (tabButton != null) {
                Log.i("nbv","tabButton----s:"+chooser.getIconResource());
                if(chooser.getIconResource() == R.drawable.act_photo){
                    Log.i("nbv","tabButton----s:true");
                    tabButton.requestFocus();
                }
                tabButton.setVisibility(enabled ? View.VISIBLE : View.GONE);
    ...
    public void dismiss(final boolean animate) {
        ...
        //juphoon修改建议操作选中录制视频 音频时 按钮不置灰的问题
        if (mSelectedChooser != null) {
    ...
    // juphoon
    public void dispatchOnExternalViewListener(final ExternalViewController externalViewListener) {
        if (mListener != null) {
            mListenerHandler.post(new Runnable() {
                @Override
                public void run() {
                    mListener.ExternalViewController(externalViewListener);
                }
            });
        }
    }
