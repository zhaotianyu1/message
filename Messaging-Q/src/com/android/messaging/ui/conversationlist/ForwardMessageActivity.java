...
public class ForwardMessageActivity extends BaseBugleActivity
    ...
    public void onConversationClick(final ConversationListData listData,
        ...
        /** juphoon 彩信不能转发到pc，公众账号，群 **/
        if (mDraftMessage.getIsMms() &&
                (conversationListItemData.getRmsThreadType() == RmsDefine.RMS_GROUP_THREAD ||
                TextUtils.equals(conversationListItemData.getOtherParticipantNormalizedDestination(), RmsDefine.PC_ADDRESS))) {
            UiUtils.showToastAtBottom(R.string.not_support_send_mms);
        } else {
            //  juphoon　转发的是彩信，存彩信标识
            if (mDraftMessage.getIsMms()) {
                RcsMmsUtils.setMmsModel(conversationListItemData.getConversationId(), true);
            }
            UIIntents.get().launchConversationActivity(this, conversationListItemData.getConversationId(), mDraftMessage);
            finish();
        }
    }
