...
public class ConversationListItemView extends FrameLayout implements OnClickListener,
    ...
    private final OnClickListener fullScreenPreviewClickListener = new OnClickListener() {
        ...
        public void onClick(final View v) {
            ...
            Assert.isTrue(ContentType.isImageType(previewType) ||
                    ...
                    ContentType.isFileType(previewType) ||
                    ContentType.isGeoType(previewType));//juphoon
			...
			//juphoon
            } else if (ContentType.isFileType(previewType)) {
                RcsFileAttachmentView.openFile(mContext, new File(RcsFileUtils.getFilePathByUri(mContext, previewUri)));
            } else if (ContentType.isGeoType(previewType)) {
                RcsBaiduLocationActivity.startWithLocation(mContext, previewUri);
            } else {
                Uri uri = previewUri;
                if (previewUri.getScheme().equals("file")) {
                    uri = RcsMmsUtils.getVideoContentUri(mContext, previewUri.getPath());
                }
                mHostInterface.startFullScreenVideoViewer(uri);
            }
        }
    };
    ...
    private void setConversationName() {
        ...
        Log.i("ooo","setConversationName-----conversationName--:"+conversationName);
        ...
        final CharSequence ellipsizedName = UiUtils.commaEllipsize(
                ...
                getPlusOneString(),
                getPlusNString());
        Log.i("ooo","setConversationName-----ellipsizedName--:"+ellipsizedName);
        ...
        Log.i("ooo","setConversationName-----bidiFormattedName--:"+bidiFormattedName);
    ...
    private void setSubject() {
      ...
      //  Log.i("mmm","subjectText---:"+subjectText);
    ...
    public void bind(final Cursor cursor, final HostInterface hostInterface) {
        ...
        mData.bind(cursor);
        Log.i("ooo","mData.get----:"+mData.getConversationId());
        resetAnimatingState();
        ...
        if (previewUri != null && !TextUtils.isEmpty(previewContentType)) {
            if (ContentType.isAudioType(previewContentType)
                    // 过滤Rcs语音未下载完成情况 juphoon
                    && !previewUri.toString().equals("file://")) {
                ...
                audioPreviewVisiblity = VISIBLE;
                    ...
                    // 过滤Rcs视频未下载完成情况 juphoon
                    && !previewUri.toString().endsWith(".thumb")) {
			...
			//juphoon
                previewClickListener = fullScreenPreviewClickListener;
                previewImageVisibility = VISIBLE;
            } else if (ContentType.isFileType(previewContentType)
                    && !previewUri.toString().equals("file://")) {
                previewImageUri = Uri.parse("android.resource://" + mContext.getPackageName() + "/" +R.drawable.icon_file);
                previewClickListener = fullScreenPreviewClickListener;
                previewImageVisibility = VISIBLE;
            } else if (ContentType.isGeoType(previewContentType)
                    && !previewUri.toString().equals("file://")) {
                previewImageUri = Uri.parse("android.resource://" + mContext.getPackageName() + "/" +R.drawable.geo_bg);
                previewClickListener = fullScreenPreviewClickListener;
                previewImageVisibility = VISIBLE;
            }
        }
        ...
        setBackgroundResource(R.drawable.blackgrounds);
        // juphoon 置顶显示
//        setBackgroundColor(mData.getPriority() > 0 ? getResources().getColor(R.color.stick_top_background_color)
//                : getResources().getColor(R.color.conversation_background));

        RcsRecipientHelper.checkRcsIcon(mData.getOtherParticipantNormalizedDestination());
    }
    ...
    public void setSwipeTranslationX(final float translationX) {
        ...
        if (translationX == 0) {
          ...
          //  mSwipeableContainer.setBackgroundColor(R.color.black3);
        } else {
    ...
    public boolean onLongClick(final View v) {
        return processClick(v, false);

    }

    private boolean processClick(final View v, final boolean isLongClick) {
        //能力查询
      //  RcsCallWrapper.rcsGetCap("sip:+86125600400001002@www.rcsrunner.cmri.cn");
        Assert.isTrue(v == mSwipeableContainer || v == mContactIconView || v == mImagePreviewView);
        Assert.notNull(mData.getName());
        Log.i("cccc","mData.getName()--:"+mData.getName());
        BugleApplication.getInstance().setName(mData.getName());
    ...
    private String getSnippetText() {
       ...
       // Log.i("mmm","snippetText---:"+snippetText);
        final String previewContentType = mData.getShowDraft() ?
                mData.getDraftPreviewContentType() : mData.getPreviewContentType();
      //  Log.i("mmm","previewContentType---:"+previewContentType);
