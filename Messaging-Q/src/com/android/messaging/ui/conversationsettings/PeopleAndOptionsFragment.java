...
import com.juphoon.helper.mms.ui.RcsSingleCreateCroupActivity;
...
public class PeopleAndOptionsFragment extends Fragment
    ...
    // juphoon
    private List<ParticipantData> mParticipants;
    private LinearLayout mOneToGroupView;
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container,
        ...
        // juphoon
        mOneToGroupView = (LinearLayout) view.findViewById(R.id.one_to_greoup_view);
        mOneToGroupView.setOnClickListener(new View.OnClickListener() {
    ...
    public void onParticipantsListLoaded(final PeopleAndOptionsData data,
        ...
        if (mParticipants != null && mParticipants.size() == 1) {
            mOneToGroupView.setVisibility(View.VISIBLE);
        } else {
            mOneToGroupView.setVisibility(View.GONE);
        }
    }

    private void createGroup() {
        if (RcsServiceManager.isLogined()) {
            int minSelectPhone = TextUtils.equals(RcsServiceManager.getUserName(), mParticipants.get(0).getNormalizedDestination()) ? 2 : 1;
            Intent intent = new Intent(getContext(), RcsSingleCreateCroupActivity.class);
            ...
            intent.putExtra(RcsCreateCroupActivity.LIMIT_MIN_SELECT, minSelectPhone);
            intent.putExtra(RcsCreateCroupActivity.LIMIT_MAX_SELECT, RcsServiceManager.getGroupMaxMemberSize() - (3 - minSelectPhone));
            intent.putExtra(RcsCreateCroupActivity.EXCLUDE_PHONES, mParticipants.get(0).getNormalizedDestination());
