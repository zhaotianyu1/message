...
public class AudioAttachmentView extends LinearLayout {
    ...
    private void onAudioReplayError(final int what, final int extra, final Exception exception) {
        ...
        // juphoon 如果是rcs消息并且未下载完成的不提示播放错误
        if (mData != null && mData.getIsRms() && mData.getIsIncoming() && mData.getStatus() != MessageData.BUGLE_STATUS_INCOMING_COMPLETE) {
            return;
        }
    ...
    private void setupMediaPlayer() {
        ...
        if (mMediaPlayer == null) {
            ...
            Log.i("zty","mDataSourceUri--:"+mDataSourceUri);
            Log.i("zty","Factory.get().getApplicationContext()--:"+Factory.get().getApplicationContext().toString());
    ...
    // juphoon
    public void bindMessagePartData(final ConversationMessageData data, final MessagePartData messagePartData, final boolean showAsSelected) {
        Assert.isTrue(messagePartData == null || messagePartData.isAudio());
        final Uri contentUri = (messagePartData == null) ? null : messagePartData.getContentUri();
        mData = data;
        bind(contentUri, mData.getIsIncoming(), showAsSelected);
    }
}
