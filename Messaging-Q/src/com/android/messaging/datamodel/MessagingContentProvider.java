...
public class MessagingContentProvider extends ContentProvider {
    ...
    // juphoon
    private static final int ALL_CONVERSATION_MESSAGES_QUERY_CODE = 1000;
    ...
    static {
        ...
        // juphoon 所有消息查询
        sURIMatcher.addURI(AUTHORITY, MESSAGES_QUERY + "/conversation",
                ALL_CONVERSATION_MESSAGES_QUERY_CODE);
    }
    ...
    public Cursor query(final Uri uri, final String[] projection, String selection,
        ...
        switch (match) {
            ...
            case DRAFT_IMAGES_QUERY_CODE:
                ...
                break;
            // juphoon
            case ALL_CONVERSATION_MESSAGES_QUERY_CODE:
                if (uri.getPathSegments().size() == 2 &&
                    TextUtils.equals(uri.getPathSegments().get(1), "conversation")) {
                    return queryConversationMessages(selection, selectionArgs, uri);
                } else {
                    throw new IllegalArgumentException("Malformed URI " + uri);
                }
            default: {
    ...
    // juphoon 查询所有ConversationMessage
    private Cursor queryConversationMessages(String selection, String[] selectionArgs, final Uri notifyUri) {
        final Cursor cursor = getDatabaseWrapper().rawQuery(
                ConversationMessageData.getConversationMessagesQuerySql(selection), selectionArgs);
        cursor.setNotificationUri(getContext().getContentResolver(), notifyUri);
        return cursor;
    }
}
