...
public class ParticipantRefresh {
    ...
    public static boolean refreshParticipant(final DatabaseWrapper db,
		...
		//juphoon
        int ret = refreshFromRcs(participantData);
        if (ret == -1) {
     ...
     * juphoon
     *
     * Rcs业务相关更新，包括群，国标Maap，企标Maap，智能短信
     * @param participantData
     * @return -1 表示不属于 Rcs 相关号码，0 表示属于但未更新，1表示更新
     */
    private static int refreshFromRcs(final ParticipantData participantData) {
        final RcsChatbotHelper.RcsChatbot chatbot = RcsChatbotHelper.getChatbotInfoByServiceId(participantData.getNormalizedDestination());
        final RcsGroupHelper.RcsGroupInfo groupInfo = RcsGroupHelper.getGroupInfo(participantData.getNormalizedDestination());
        final String currentDisplayName = participantData.getFullName();
        final String currentPhotoUri = participantData.getProfilePhotoUri();
        String name = null, logo = null;
        // 只有 serviceId 的 chatbot 才更新头像和名称
        if (chatbot != null) {
            name = chatbot.name;
            logo = RcsFileDownloadHelper.getPathFromFileInfo(new RcsFileDownloadHelper.FileInfo(chatbot.icon), RmsDefine.RMS_ICON_PATH);
            if (logo == null) {
            ...
            name = groupInfo.mSubject;
            logo = new StringBuffer()
                    .append("android.resource://").append(RcsMmsInitHelper.getContext().getPackageName())
                    .append("/").append(R.drawable.ic_rcs_group).toString();
        } else if (TextUtils.equals(RmsDefine.PC_ADDRESS, participantData.getNormalizedDestination())) {
            name = RcsMmsInitHelper.getContext().getString(R.string.my_pc);
            logo = new StringBuffer()
                    .append("android.resource://").append(RcsMmsInitHelper.getContext().getPackageName())
                    .append("/").append(R.drawable.ic_computer).toString();
        } else {
            if (participantData.getContactId() == ParticipantData.PARTICIPANT_CONTACT_ID_RCS) {
                participantData.setContactId(ParticipantData.PARTICIPANT_CONTACT_ID_NOT_FOUND);
                participantData.setFullName(null);
                participantData.setFirstName(null);
                participantData.setProfilePhotoUri(null);
        ...
        if (!TextUtils.isEmpty(logo) && !logo.startsWith("android.resource")) {
            logo = Uri.fromFile(new File(logo)).toString();
        }
        if (!TextUtils.equals(name, currentDisplayName) || !TextUtils.equals(logo, currentPhotoUri)) {
            participantData.setContactId(ParticipantData.PARTICIPANT_CONTACT_ID_RCS);
            participantData.setFullName(name);
            participantData.setFirstName(name);
            participantData.setProfilePhotoUri(logo);
            return 1;
        }
        return 0;
    }
    ...
    /**
     * rcs 强制同步，maap头像信息更新等等
     */
    public static void rcsForceRefresh() {
        if (sFullRefreshScheduled.compareAndSet(false, true)) {
            if (LogUtil.isLoggable(TAG, LogUtil.VERBOSE)) {
                LogUtil.v(TAG, "Started full participant refresh");
            }
            SafeAsyncTask.executeOnThreadPool(sFullRefreshRunnable);
