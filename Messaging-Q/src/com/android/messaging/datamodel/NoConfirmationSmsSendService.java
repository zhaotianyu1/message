    ...
    protected void onHandleIntent(final Intent intent) {
        ...
        if (extras.getBoolean("showUI", false)) {
            ...
            if (TextUtils.isEmpty(conversationId)) {
                ...
                //Juhoon 通知栏回复逻辑,判断subId
                int mSubId;
                DatabaseWrapper db = DataModel.get().getDatabase();
                ParticipantData unboundSelf = BugleDatabaseOperations.getExistingParticipant(
                        db, selfId);
                if (unboundSelf.getSubId() == ParticipantData.DEFAULT_SELF_SUB_ID
                    ...
                    ParticipantData self = BugleDatabaseOperations.getOrCreateSelf(db, defaultSubId);
                    mSubId = self.getSubId();
                } else {
                    ParticipantData self = unboundSelf;
                    mSubId = self.getSubId();
                }
                if (RcsServiceManager.isLogined() && (mSubId == ParticipantData.DEFAULT_SELF_SUB_ID || mSubId == RcsServiceManager.getSubId())) {
                    messageData = MessageData.createDraftRmsMessage(conversationId, selfId, message);
                } else {
                    if (requiresMms) {
                        if (LogUtil.isLoggable(TAG, LogUtil.VERBOSE)) {
