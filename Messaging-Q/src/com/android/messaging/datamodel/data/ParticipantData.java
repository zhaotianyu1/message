...
public class ParticipantData implements Parcelable {
    ...
    /**
     * juphoon
     * 被 Rcs 相关业务进行头像和名称的处理
     */
    public static final long PARTICIPANT_CONTACT_ID_RCS = -10;
    ...
    public ContentValues toContentValues() {
        ...
        if (!isUnknownSender()) {
			...
			//juphoon
            String name = null;
            if (RcsGroupHelper.isGroupChatId(mNormalizedDestination, false)) {
                RcsGroupHelper.RcsGroupInfo groupInfo = RcsGroupHelper.getGroupInfo(mNormalizedDestination);
                if (groupInfo != null) {
                    name = TextUtils.isEmpty(groupInfo.mGroupNickName) ?
                            groupInfo.mSubject : groupInfo.mGroupNickName;
                } else {
                    name = mNormalizedDestination;
                }
                values.put(ParticipantColumns.FULL_NAME, name);
                values.put(ParticipantColumns.FIRST_NAME, name);
            } else if (TextUtils.equals(mNormalizedDestination, RmsDefine.PC_ADDRESS)) {
                values.put(ParticipantColumns.FULL_NAME, Factory.get().getApplicationContext().getResources().getString(R.string.my_pc));
                values.put(ParticipantColumns.FIRST_NAME, Factory.get().getApplicationContext().getResources().getString(R.string.my_pc));
            } else {
