...
public class MessageData implements Parcelable {
    ...
    //多条RcsMessage时标记是否为最后一条、用于更新draft
    private int mIsEnd; // 1表示可更新 , 0标示不能更新
    ...
    // juphoon
    public static final int PROTOCOL_RMS = 100;                   // RMS message
    ...
    // juphoon
    public static final int BUGLE_STATUS_OUTGOING_DISPLAYED               = 10; //RMS displayed
    ...
    public static final String getStatusDescription(int status) {
        switch (status) {
            ...
            case BUGLE_STATUS_OUTGOING_DISPLAYED:
    ...
    public void setRcsMessageIsEnd(boolean isEnd) {
        mIsEnd = isEnd ? 1 : 0;
    }
    public boolean getRcsMessageIsEnd() {
        return mIsEnd == 1;
    }
    ...
    public final String getMessageText() {
        ...
        for (final MessagePartData part : mParts) {
            ...
            } else if (part.isChatbotCard()) {
                text.append(part.getContentUri().getAuthority());
            }
        }
    ...
    public void writeToParcel(final Parcel dest, final int flags) {
        ...
        dest.writeInt(mIsEnd);  //juphoon add
        // Write parts
    ...
    /** juphoon **/
    public static MessageData createDraftRmsMessage(final String conversationId,
            final String selfId, final String messageText) {
        final MessageData message = new MessageData();
        message.mStatus = BUGLE_STATUS_OUTGOING_DRAFT;
        message.mProtocol = PROTOCOL_RMS;
        message.mConversationId = conversationId;
        message.mParticipantId = selfId;
        message.mSelfId = selfId;
        message.mReceivedTimestamp = System.currentTimeMillis();
    ...
    //juphoon 创建建议回复
    public static MessageData createChatbotReplyMessage(final String conversationId, final String selfId, final String messageText, final String reply, final String contributionId, final String trafficType) {
        final MessageData message = new MessageData();
        message.mProtocol = PROTOCOL_RMS;
        message.mConversationId = conversationId;
        message.mParticipantId = selfId;
        ...
        message.mReceivedTimestamp = System.currentTimeMillis();
        if (!TextUtils.isEmpty(messageText)) {
            MessagePartData messagePartData = MessagePartData.createTextMessagePart(messageText);
            messagePartData.mExtra = reply;
            messagePartData.mContributionId = contributionId;
            messagePartData.mTrafficType = trafficType;
    ...
    /** juphoon **/
    public static MessageData createReceivedRmsMessage(final Uri uri, final String conversationId,
                                                       final String participantId, final String selfId,
                                                       final boolean seen, final boolean read, RcsDatabaseMessages.RmsMessage rms) {
        final MessageData message = new MessageData();
        message.mSmsMessageUri = uri;
        message.mConversationId = conversationId;
        message.mParticipantId = participantId;
        message.mSelfId = selfId;
        message.mProtocol = PROTOCOL_RMS;
        if (!TextUtils.isEmpty(rms.mTransId) && rms.mStatus <= RmsDefine.Rms.STATUS_PENDING) {
            message.mStatus = BUGLE_STATUS_INCOMING_YET_TO_MANUAL_DOWNLOAD;
        } else {
            message.mStatus = BUGLE_STATUS_INCOMING_COMPLETE;
        }
        if ((rms.mMixType & RmsDefine.Rms.MIX_TYPE_CC) > 0) {
            message.mParticipantId = selfId;
        }
        message.mMmsTransactionId = rms.mTransId;
        message.mReceivedTimestamp = rms.mTimestampInMillis;
        message.mSentTimestamp = rms.mTimestampSentInMillis;
        message.mSeen = seen;
        message.mRead = read;
        if (rms.mMessageType == RmsDefine.Rms.RMS_MESSAGE_TYPE_TEXT || rms.mMessageType == RmsDefine.Rms.RMS_MESSAGE_TYPE_SYSTEM
                || rms.mMessageType == RmsDefine.Rms.RMS_MESSAGE_TYPE_CHATBOT_TEXT) {
            message.mParts.add(MessagePartData.createTextMessagePart(rms.mBody));
        }
        return message;
    }

    /** juphoon
     * Create a message from Rms table fields
     */
    public static MessageData createRmsMessage(final String messageUri, final String participantId, final String selfId,
                                               final String transId, final String conversationId, final int bugleStatus,
                                               final boolean seen, final boolean read, final long sent, final long recieved,
                                               final String messageText, final String type) {
        final MessageData message = new MessageData();
        message.mParticipantId = participantId;
        message.mSelfId = selfId;
        message.mMmsTransactionId = transId;
        message.mConversationId = conversationId;
        message.mSentTimestamp = sent;
        message.mReceivedTimestamp = recieved;
        message.mSeen = seen;
        ...
        message.mStatus = bugleStatus;
        message.mSmsMessageUri = Uri.parse(messageUri);
        if (!TextUtils.isEmpty(messageText) && androidx.appcompat.mms.pdu.ContentType.isTextType(type)) {
            message.mParts.add(MessagePartData.createTextMessagePart(messageText));
        }
        return message;
    }

    public final boolean getIsRms() {
        return mProtocol == MessageData.PROTOCOL_RMS;
    }

    public final void updateMessageDownLoadStatus(int status) {
        mStatus = status;
    }

    static boolean getIsProgress(final int status) {
        return (status == BUGLE_STATUS_OUTGOING_SENDING ||
                status == BUGLE_STATUS_INCOMING_AUTO_DOWNLOADING ||
                status == BUGLE_STATUS_INCOMING_MANUAL_DOWNLOADING);
    }

    public final void updateMessageStatus(int status) {
        mStatus = status;
    }
}
