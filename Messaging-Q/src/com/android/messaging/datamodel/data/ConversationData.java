...
public class ConversationData extends BindableData {
    ...
    public MessageData createForwardedMessage(final ConversationMessageData message) {
        ...
        /** juphoon 转发一条彩信 **/
        if (message.getIsMms() && message.getParts().size() > 0) {
            forwardedMessage = new MessageData(message.getSmsMessageUri(), MessageData.PROTOCOL_MMS);
        } else {
    ...
    // juphoon
    public int getConversationThreadType() {
        return mConversationMetadata.getRmsThreadType();
    }
    ...
    public long getPriority() {
        return mConversationMetadata.getPriority();
    }

    // juphoon
    public boolean isPcConversation() {
        return TextUtils.equals(mConversationMetadata.getOtherParticipantNormalizedDestination(), RmsDefine.PC_ADDRESS);
    }

    // juphoon
    public String getDraftSnippetText() {
        return mConversationMetadata.getDraftSnippetText();
    }

    // juphoon
    public Uri getDraftPreviewUri() {
        return mConversationMetadata.getDraftPreviewUri();
    } 
}
