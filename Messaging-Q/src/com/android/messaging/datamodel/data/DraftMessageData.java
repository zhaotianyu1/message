...
public class DraftMessageData extends BindableData implements ReadDraftDataActionListener {
    ...
    public void updateFromMessageData(final MessageData message, final String bindingId) {
        ...
        /** juphoon 加载预览，读是否是彩信，是彩信设置彩信模式 **/
        if (RcsMmsUtils.getMmsModel(mConversationId)) {
            /** 转发彩信时标识类型 **/
            setMmsModel(true);
        }

        /** juphoon 加载附件为空时，会存一个空字符串的文本part，这种情况删除对应会话的彩信标识 **/
        List<MessagePartData> parts = (List<MessagePartData>) message.getParts();
        if (parts.size() == 1 &&TextUtils.equals(parts.get(0).getContentType(), androidx.appcompat.mms.pdu.ContentType.TEXT_PLAIN) &&
                TextUtils.isEmpty(parts.get(0).getText())) {
            RcsMmsUtils.removeMmsModel(mConversationId);
        }
    ...
    public boolean getIsMms() {
        ...
        /** juphoon 增加彩信判断 **/
        return mIsMmsModel || (MmsSmsUtils.getRequireMmsForEmailAddress(mIncludeEmailAddress, selfSubId) && !mIsChatbot) ||
    ...
    public boolean containsAttachment(final Uri contentUri) {
        /** juphoon contentUri为file://是表情，位置，红包类型，这里对这些类型不做重复的判断 **/
        if (TextUtils.equals(contentUri.toString(), "file://")) {
            return false;
        }
    ...
    public void onReadDraftDataSucceeded(final ReadDraftDataAction action, final Object data,
        ...
        if (isBound(bindingId)) {
            ...
            mIsChatbot = RcsChatbotHelper.isChatbotByServiceId(conversation.getOtherParticipantNormalizedDestination());//juphoon
    ...
    // juphoon
    public ArrayList<MessageData> prepareMessageForRmsSendingList(final BindingBase<DraftMessageData> binding) {
        Assert.isTrue(!hasPendingAttachments());
        mSending = true;
        final ArrayList<MessageData> messageToSend = createRmsMessageWithCurrentAttachments(true);
        mSending = false;
        return messageToSend;
    }

    // juphoon 将多附件消息转化成每条rcs消息
    private ArrayList<MessageData> createRmsMessageWithCurrentAttachments(final boolean clearLocalCopy) {
        ...
        if (RcsServiceManager.isLogined()) {
            if (!TextUtils.isEmpty(mMessageText)) {
                MessageData message = MessageData.createDraftRmsMessage(mConversationId, mSelfId, mMessageText);
                messageList.add(message);
            }
            for (final MessagePartData attachment : mAttachments) {
                MessageData message = MessageData.createDraftRmsMessage(mConversationId, mSelfId, null);
                message.addPart(attachment);
                messageList.add(message);
            }
        }
    ...
    // juphoon 设置彩信模式
    public void setMmsModel(boolean isMmsModel) {
        RcsMmsUtils.setMmsModel(mConversationId, isMmsModel);
        mIsMmsModel = isMmsModel;
    }

    //juphoon
    public boolean getMmsModel() {
        return mIsMmsModel || !TextUtils.isEmpty(mMessageSubject);
    }
