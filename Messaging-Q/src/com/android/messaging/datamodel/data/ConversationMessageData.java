...
public class ConversationMessageData {
    ...
    // juphoon
    private int mConversationRmsThreadType;
    private String mConversationParticipantNormalizedDestination;
    ...
    public void bind(final Cursor cursor) {
        ...
        // juphoon
        mConversationRmsThreadType = cursor.getInt(INDEX_CONVERSATION_RMS_THREAD_TYPE);
        mConversationParticipantNormalizedDestination = cursor.getString(INDEX_CONVERSATION_PARTICIPANT_NORMALIZED_DESTINATION);
    ...
    private boolean canClusterWithMessage(final Cursor cursor) {
        ...
        final boolean otherIsIncoming = (otherStatus >= MessageData.BUGLE_STATUS_FIRST_INCOMING && (!mSelfParticipantId.equals(mParticipantId)));//juphoon
    ...
    static MessagePartData makePartData(
        ...
        if (ContentType.isTextType(contentType)) {
            ...
            final Uri contentUri = TextUtils.isEmpty(contentUriString) ? Uri.parse("file://") : Uri.parse(contentUriString);//juphoon
            ...
            /** 当为文件消息是 text 字段保存文件文件名
             * 当为智能消息 text 字段保存 xml juphoon
             */
            if (ContentType.isFileType(contentType) || ContentType.isChatbotCard(contentType)) {
    ...
    public boolean getIsIncoming() {
        //juphoon
        return (mStatus >= MessageData.BUGLE_STATUS_FIRST_INCOMING && (!mSelfParticipantId.equals(mParticipantId)));//juphoon
    }
    ...
    public boolean getCanForwardMessage() {
        ...
        return (mStatus == MessageData.BUGLE_STATUS_OUTGOING_DELIVERED ||
                mStatus == MessageData.BUGLE_STATUS_OUTGOING_DISPLAYED ||
                mStatus == MessageData.BUGLE_STATUS_INCOMING_COMPLETE); //juphoon
    }
    ...
    public static final String getWearableQuerySql() {
        return CONVERSATION_MESSAGES_QUERY_SQL
                ...
                + MessageData.BUGLE_STATUS_OUTGOING_DISPLAYED + ", "
    ...
    private static final String CONVERSATION_MESSAGES_QUERY_PROJECTION_SQL =
            ...
            // juphoon
            + DatabaseHelper.CONVERSATIONS_TABLE + '.' + DatabaseHelper.ConversationColumns.RMS_THREAD_TYPE
            + " as " + ConversationMessageViewColumns.CONVERSATION_RMS_THREAD_TYPE + ", "
            + DatabaseHelper.CONVERSATIONS_TABLE + '.' + DatabaseHelper.ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION
            + " as " + ConversationMessageViewColumns.CONVERSATION_PARTICIPANT_NORMALIZED_DESTINATION + " ";
    ...
    interface ConversationMessageViewColumns extends BaseColumns {
        ...
        // juphoon 增加每条消息对应的address和threadType
        static final String CONVERSATION_PARTICIPANT_NORMALIZED_DESTINATION = DatabaseHelper.ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION;
        static final String CONVERSATION_RMS_THREAD_TYPE = DatabaseHelper.ConversationColumns.RMS_THREAD_TYPE;
    }
    ...
    // juphoon
    private static final int INDEX_CONVERSATION_RMS_THREAD_TYPE  = sIndexIncrementer++;
    private static final int INDEX_CONVERSATION_PARTICIPANT_NORMALIZED_DESTINATION    = sIndexIncrementer++;
    ...
    private static String[] sProjection = {
        ...
        // juphoon
        ConversationMessageViewColumns.CONVERSATION_RMS_THREAD_TYPE,
        ConversationMessageViewColumns.CONVERSATION_PARTICIPANT_NORMALIZED_DESTINATION,
    };
    ...
    // juphoon
    public final int getConversationRmsThreadType() {
        return mConversationRmsThreadType;
    }

    // juphoon
    public final String getConversationParticipantNormalizedDestination() {
        return mConversationParticipantNormalizedDestination;
    }
    
    // juphoon 根据条件查询所有ConversationMessage
    public static final String getConversationMessagesQuerySql(String selsction) {
        return CONVERSATION_MESSAGES_QUERY_SQL
                + (TextUtils.isEmpty(selsction) ? "" : " AND ")
                + selsction +")"
                + CONVERSATION_MESSAGES_QUERY_SQL_GROUP_BY;
    }
    ...
    public boolean getIsProgress(){
        return MessageData.getIsProgress(mStatus);
    }
