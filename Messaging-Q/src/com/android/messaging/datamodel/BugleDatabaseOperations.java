...
public class BugleDatabaseOperations {
    ...
    public static String getOrCreateConversation(final DatabaseWrapper db, final long threadId,
        ...
        String conversationId = BugleDatabaseOperations.getExistingConversation(db, threadId,
                false);
        String name = BugleDatabaseOperations.getExistingConversationName(db,threadId);
        ...
        if (conversationId == null) {
            ...
            Log.i("MessagingApp","getOrCreateConversation:conversationName--:"+conversationName);
    ...
    @VisibleForTesting
    @DoesNotRunOnMainThread
    public static Long getExistingTime(final DatabaseWrapper dbWrapper,
                                                  final String conersionId) {
        Assert.isNotMainThread();
        long time = 0;
        Cursor cursor = null;
        try {
            ...
            cursor = dbWrapper.rawQuery("SELECT " + ConversationColumns.SORT_TIMESTAMP
                            ...
                            + " WHERE " + ConversationColumns._ID + "=" + conersionId,
                    null);
    ...
    @VisibleForTesting
    @DoesNotRunOnMainThread
    public static String getExistingName(final DatabaseWrapper dbWrapper,
                                                 final String conersionId) {
        Assert.isNotMainThread();
        String name = null;
        ...
        try {
            // Look for an existing conversation in the db with this thread id
            cursor = dbWrapper.rawQuery("SELECT " + ConversationColumns.NAME
                            + " FROM " + DatabaseHelper.CONVERSATIONS_TABLE
                            + " WHERE " + ConversationColumns._ID + "=" + conersionId,
                    null);
            if (cursor.moveToFirst()) {
                Assert.isTrue(cursor.getCount() == 1);
                name = cursor.getString(cursor.getColumnIndex(ConversationColumns.NAME));
            }
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    @VisibleForTesting
    @DoesNotRunOnMainThread
    public static List<ConversationListItemData> getExistingConversation(final DatabaseWrapper dbWrapper) {
        List<ConversationListItemData> conversationListItemData = new ArrayList<>();
//        Assert.isNotMainThread();
        String snip_text =null;
        Cursor cursor = null;
        try {
            // Look for an existing conversation in the db with this thread id
            cursor = dbWrapper.rawQuery("SELECT * "
                            + " FROM " + DatabaseHelper.CONVERSATIONS_TABLE + " order by " +
                            SORT_ORDER,
                    null);
            Log.i("ooo","getExistingConversation--:"+cursor.getCount());
            int n = 0;
            while (cursor.moveToNext()){
                ConversationListItemData conversationListItemData1 = new ConversationListItemData();
                String name = cursor.getString(cursor.getColumnIndex("name"));
                String snippet_text = cursor.getString(cursor.getColumnIndex("snippet_text"));
                String conversionId = cursor.getString(cursor.getColumnIndex("_id"));
                Long time = cursor.getLong(cursor.getColumnIndex("sort_timestamp"));
                conversationListItemData1.setmConversationId(conversionId);
                conversationListItemData1.setmName(name);
                conversationListItemData1.setmSnippetText(snippet_text);
                conversationListItemData1.setmTimestamp(time);
                Log.i("ooo","cursor.name----:"+name);
                Log.i("ooo","cursor.SNIPPET_TEXT----:"+snippet_text);
                Log.i("ooo","cursor.conversionId----:"+conversionId);
                Log.i("ooo","cursor.time----:"+time);
                conversationListItemData.add(n,conversationListItemData1);
                n++;
            }
    ...
    @VisibleForTesting
    @DoesNotRunOnMainThread
    public static String getExistingConversations(final DatabaseWrapper dbWrapper,
                                                 final String conersionId) {
        Assert.isNotMainThread();
        String snip_text =null;
        Cursor cursor = null;
        try {
            // Look for an existing conversation in the db with this thread id
            cursor = dbWrapper.rawQuery("SELECT " + ConversationColumns.SNIPPET_TEXT
                            + " FROM " + DatabaseHelper.CONVERSATIONS_TABLE
                            + " WHERE " + ConversationColumns._ID + "=" + conersionId,
                    null);
    ...
    /**
     * Juphoon Get a conversation name from the local DB based on the message's thread id.
     *
     * @param dbWrapper
     * @param threadId
     * @return
     */
    @DoesNotRunOnMainThread
    public static String getExistingConversationName(final DatabaseWrapper dbWrapper,
            final long threadId) {
        Assert.isNotMainThread();
    ...
    static String createConversationInTransaction(final DatabaseWrapper dbWrapper,
        ...
        values.put(ConversationColumns.RMS_THREAD_TYPE, RcsMmsUtils.getThreadType(threadId)); //juphoon
    ...
    public static boolean deleteConversation(final DatabaseWrapper dbWrapper,
        ...
        try {
            ...
            /**
             * juphoon 删除该会话消息对应的parts
             */
            deletePartsForConversation(dbWrapper, conversationId, cutoffTimestamp);
    ...
    public static void updateConversationMetadataInTransaction(final DatabaseWrapper dbWrapper,
        ...
        //juphoon 更新proprity
        if (getPriorityByConversationId(dbWrapper, message.getConversationId()) > 0) {
            values.put(ConversationColumns.PRIORITY, latestTimestamp);
        }
    ...
    private static void updateConversationDraftSnippetAndPreviewInTransaction(
        ...
        //juphoon 更新proprity
        if (getPriorityByConversationId(dbWrapper, conversationId) > 0) {
            values.put(ConversationColumns.PRIORITY, sortTimestamp);
        }
	...
	//juphoon
    @DoesNotRunOnMainThread
    public static String getLastMessageId(final DatabaseWrapper dbWrapper, final String conversationId) {
        Assert.isNotMainThread();
        Cursor cursor = null;
        try {
            cursor = dbWrapper.query(DatabaseHelper.CONVERSATIONS_TABLE,
    ...
    public static ArrayList<ParticipantData> getParticipantsForConversation(
            final DatabaseWrapper dbWrapper, final String conversationId) {
        Log.i("vcx","conversationId--:"+conversationId);
        Assert.isNotMainThread();
        final ArrayList<ParticipantData> participants =
     ...
     */
    //juphoon
    public static void readMessagePartsData(final DatabaseWrapper dbWrapper,
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                if (checkAttachmentFilesExist && messagePartData.isAttachment() &&
                    ...
                    try {
                        /** juphoon  位置消息草稿在退出界面后，再进入，草稿不显示 **/
                        if (ContentType.TEXT_VCARD.equalsIgnoreCase(messagePartData.getContentType())
                                || ContentType.isGeoType(messagePartData.getContentType())) {
	...
	//juphoon
    static void deletePartsForConversation(final DatabaseWrapper dbWrapper,
                                      final String conversation, final long cutoffTimestamp) {
        final int cnt;
        if (cutoffTimestamp == Long.MAX_VALUE) {
            cnt = dbWrapper.delete(DatabaseHelper.PARTS_TABLE, PartColumns.CONVERSATION_ID + " =?", new String[] { conversation });
        } else {
            cnt = dbWrapper.delete(DatabaseHelper.PARTS_TABLE, PartColumns.CONVERSATION_ID + " =? AND " + PartColumns.TIMESTAMP + " <=?", new String[] { conversation, Long.toString(cutoffTimestamp) });
        }
        Assert.inRange(cnt, 0, Integer.MAX_VALUE);
    }
    ...
    public static int deleteMessage(final DatabaseWrapper dbWrapper, final String messageId) {
        ...
        try {
            ...
            if (message != null) {
                 ...
                 * juphoon
                 */
                deletePartsForMessage(dbWrapper, messageId);
    ...
    public static boolean getConversation(final DatabaseWrapper dbWrapper
    ) {
        // Look for an existing conversation in the db with this conversation id
        Cursor cursor = null;
        try {
     ...
     * juphoon 更新会话的名字
     */
    public static void updateConversationNameInTransaction(
            final DatabaseWrapper dbWrapper, String conversationId, String name) {
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
     ...
     * juphoon 更新消息收藏状态
     */
    public static boolean updateMessageLockedInTransaction(final DatabaseWrapper dbWrapper,
            String conversationId, String msgId, boolean locked) {
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
        final ContentValues values = new ContentValues();
        boolean result = updateMessageRowIfExists(dbWrapper, msgId, values);
        MessagingContentProvider.notifyMessagesChanged(conversationId);
        return result;
    }
     ...
     * juphoon 更新会话优先级
     */
    public static void updateConversationPriorityInTransaction(final DatabaseWrapper dbWrapper,
            String conversationId, long priority) {
        Assert.isNotMainThread();
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
        final ContentValues values = new ContentValues();
        values.put(ConversationColumns.PRIORITY, priority);
        updateConversationRowIfExists(dbWrapper, conversationId, values);
        MessagingContentProvider.notifyConversationMetadataChanged(conversationId);
        MessagingContentProvider.notifyConversationListChanged();
    }
     ...
     * @param dbWrapper
     */
    public static ArrayList<String> getBlockedDestination(final DatabaseWrapper dbWrapper) {
        Assert.isNotMainThread();
        ArrayList<String> blockedDestinationList = new ArrayList<>();
        Cursor cursor = dbWrapper.query(DatabaseHelper.PARTICIPANTS_TABLE, new String[]{ParticipantColumns.NORMALIZED_DESTINATION},
            ...
            try {
                while (cursor.moveToNext()) {
                    ...
                    if (!TextUtils.isEmpty(number)) {
                        blockedDestinationList.add(RcsNumberUtils.formatPhone86(number));
                    }
                }
            } finally {
                cursor.close();
     ...
     *
     * @param dbWrapper
     * @param conversationId 填""为搜索所有会话
     * @param key            搜索关键字
     * @return 搜索结果
     */
    public static Cursor queryMessage(final DatabaseWrapper dbWrapper, String conversationId, String key) {
        Assert.isNotMainThread();
        StringBuilder selection = new StringBuilder();

        selection.append(PartColumns.TEXT + " Like " + "'%" + key + "%' escape '\\' ");
        if (!TextUtils.isEmpty(conversationId)) {
            selection.append(" AND " + PartColumns.CONVERSATION_ID + " = " + conversationId);
        }
        Assert.isTrue(dbWrapper.getDatabase().inTransaction());
        return dbWrapper.query(DatabaseHelper.PARTS_TABLE,
                new String[]{PartColumns._ID, PartColumns.MESSAGE_ID, PartColumns.TEXT, PartColumns.CONVERSATION_ID, PartColumns.CONTENT_TYPE, PartColumns.TIMESTAMP}
                , selection.toString(), null, null, null, PartColumns.TIMESTAMP);

    }

    /**
     * Juphoon Get a participant_normalized_destination from the local DB based on the message's thread id.
     *
     * @param dbWrapper
     * @param threadId
     * @return
     */
    @DoesNotRunOnMainThread
    public static String getConversationParticipantNormalizedDestination(final DatabaseWrapper dbWrapper, final long threadId) {
        Assert.isNotMainThread();
        String partPnormalizedDestination = null;
        Cursor cursor = null;
        try {
            // Look for an existing conversation in the db with this thread id
            cursor = dbWrapper.rawQuery("SELECT " + ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION
                            + " FROM " + DatabaseHelper.CONVERSATIONS_TABLE
                            + " WHERE " + ConversationColumns.SMS_THREAD_ID + "=" + threadId,
                    null);
            if (cursor.moveToFirst()) {
                Assert.isTrue(cursor.getCount() == 1);
                partPnormalizedDestination = cursor.getString(0);
            }
        } finally {
            if (cursor != null) {
                cursor.close();
        ...
        return partPnormalizedDestination;
    }

    /**
     * Juphoon 根据conversationId得到priority
     *
     * @return
     */
    public static long getPriorityByConversationId(DatabaseWrapper dbWrapper, String conversationId) {
        Cursor cursor = null;
        try {
            cursor = dbWrapper.query(DatabaseHelper.CONVERSATIONS_TABLE, new String[]{ConversationColumns.PRIORITY},
    ...
    /**
     * Juphoon 删除同步表dirty消息
     *
     * @return
     */
    @DoesNotRunOnMainThread
    public static void deleteDirtyMessage(DatabaseWrapper dbWrapper, String messageUri) {
        if (!TextUtils.isEmpty(messageUri)) {
            Assert.isNotMainThread();
            dbWrapper.beginTransaction();
            Cursor cursor = null;
            try {
                cursor = dbWrapper.query(DatabaseHelper.MESSAGES_TABLE, new String[]{MessageColumns._ID, MessageColumns.CONVERSATION_ID},
                        MessageColumns.SMS_MESSAGE_URI + "=?", new String[]{messageUri}, null, null, null);
                while (cursor.moveToNext()) {
                    String messageId = cursor.getString(0);
                    String conversationId = cursor.getString(1);
                    dbWrapper.delete(DatabaseHelper.MESSAGES_TABLE, MessageColumns._ID + "=?", new String[]{messageId});
                    deletePartsForMessage(dbWrapper, messageId);
                    if (!deleteConversationIfEmptyInTransaction(dbWrapper, conversationId)) {
                        refreshConversationMetadataInTransaction(dbWrapper, conversationId,
                                false/* shouldAutoSwitchSelfId */, false/*archived*/);
