...
public abstract class MessageNotificationState extends NotificationState {
    ...
    public static class MultiMessageNotificationState extends MessageNotificationState {
        ...
        public MultiMessageNotificationState(final ConversationInfoList convList) {
            ...
            if (mAttachmentUri != null) {
                ...
                /** juphoon 增加地理位置和文件 */
                else if (ContentType.isGeoType(mAttachmentType)) {
                    message = R.string.notification_geo;
                }
                else if (ContentType.isFileType(mAttachmentType)) {
                    message = R.string.notification_file;
                }
                /** 增加智能消息获得title的逻辑 */
                final String attachment;
                if (ContentType.isChatbotCard(mAttachmentType)) {
                    attachment = mAttachmentUri.getAuthority();
                } else {
                    attachment = context.getString(message);
                }
                ...
                if (mAttachmentUri != null && shouldShowImage) {
                    ...
                    final CharSequence tickerTag =
                            BugleNotifications.formatAttachmentTag(authorFirstName,
                                    mAttachmentType, mAttachmentUri);//juphoon
            ...
            if (convMessageCursor != null && convMessageCursor.moveToFirst()) {
                ...
                do {
                    ...
                    if (currConvInfo.mTotalMessageCount < maxMessages) {
                        ...
                        Uri attachmentUri = null;
                        String attachmentType = null;
                        final MessagePartData messagePartData = getMostInterestingAttachment(convMessageData);//juphoon
    ...
    private static MessagePartData getMostInterestingAttachment(
        ...
        /** juphoon */
        MessagePartData geoPart = null;
        MessagePartData filePart = null;
        MessagePartData publicPart = null;
        MessagePartData chatbotPart = null;
        ...
        for (final MessagePartData messagePartData : attachments) {
            ...
            /** juphoon 增加地理位置通知 */
            if (messagePartData.isGeo() && geoPart == null) {
                geoPart = messagePartData;
            }
            /** juphoon 增加文件通知 */
            if (messagePartData.isFile() && filePart == null) {
                filePart = messagePartData;
            }

            /** juphoon 增加智能通知 */
            if (messagePartData.isChatbotCard() && chatbotPart == null) {
                chatbotPart = messagePartData;
            }
        }
        if (imagePart != null) {
            return imagePart;
