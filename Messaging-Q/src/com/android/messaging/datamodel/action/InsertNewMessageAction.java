...
import com.juphoon.helper.mms.RcsGroupAtHelper;
import com.juphoon.helper.mms.RcsMessage;
import com.juphoon.helper.mms.RcsMessageSendService;
import com.juphoon.helper.mms.RcsMessageSender;
...
public class InsertNewMessageAction extends Action implements Parcelable {
    ...
    protected Object executeAction() {
        ...
        String obj2=gson2.toJson(message);
        Log.i("vcx","obj2-----:"+obj2);
        if (message == null) {
        ...
        // juphoon
        final boolean isRms = (message.getProtocol() == MessageData.PROTOCOL_RMS);
        if (isRms) {
            insertRmsMessage(conversationId, message, subId, recipients);
            // Can now clear draft from conversation (deleting attachments if necessary)
            if (message.getRcsMessageIsEnd()) {
    ...
    private MessageData createMessage() {
        ...
        // juphoon 只有在通知栏直接回复消息时,如果intent没有携带conversationId时会使用
        if (RcsServiceManager.isLogined()) {
            return MessageData.createDraftRmsMessage(conversationId, self.getId(), messageText);
    ...
    private void insertBroadcastSmsMessage(final String conversationId,
        ...
        LogUtil.v(TAG, "InsertNewMessageAction: address "
                + address);
    ...
    private MessageData insertSendingSmsMessage(final MessageData content, final int subId,
        ...
        /**
         * juphoon
         * 如果是 chatbot 发送短信，则要将 serviceId 替换为短信号码
         */
        String smsAddress = recipient;
        RcsChatbotHelper.RcsChatbot chatbot = RcsChatbotHelper.getChatbotInfoByServiceId(recipient);
        if (chatbot != null) {
            if (TextUtils.isEmpty(chatbot.sms)) {
                Log.d(TAG, "chatbot send sms fail(no sms)");
                return null;
            } else {
                smsAddress = chatbot.sms;
            }
        }
    ...
    // juphoon 插入数据库操作
    private void insertRmsMessage(final String conversationId,
            final MessageData message, final int subId,
            final ArrayList<String> recipients) {
        final Context context = Factory.get().getApplicationContext();
        final DatabaseWrapper db = DataModel.get().getDatabase();
        ...
        String rmsAddress = address;
        if (recipients.size() > 1) {
            rmsAddress = TextUtils.join(";", recipients);
        }
        int rmsType = 0;
        ConversationListItemData item = ConversationListItemData.getExistingConversation(db, conversationId);
        if (item != null) {
            rmsType = item.getRmsThreadType();
        }
        Iterable<MessagePartData> parts = message.getParts();
        for (MessagePartData part : parts) {
            RcsMessage rcsMsg = null;
            if (part.isGeo()) {
                String filePath = RcsFileUtils.getFilePathByUri(context, part.getContentUri());
                if (!TextUtils.isEmpty(filePath)) {
                    rcsMsg = new RcsMessage(getRmsTypeFromMessagePartData(part), RcsMmsUtils.geoFileToString(filePath), Uri.fromFile(new File(filePath)));
                }
            } else if (part.isMedia()) {
                String path = RcsFileUtils.getFilePathByUri(context, part.getContentUri());
                File file = null;
                if (TextUtils.isEmpty(path)) {
                    file = MediaScratchFileProvider.getFileFromUri(part.getContentUri());
                } else {
                    file = new File(path);
                }
                // 如果获得的文件是这个包下面的则需要拷贝出来，否则service读取不了
                if (file.getAbsolutePath().contains(context.getPackageName())) {
                    file = new File(RcsFileUtils.getNotExistFilePath(RmsDefine.RMS_FILE_PATH, file.getName()));
                    try {
                        RcsMmsUtils.copyToFile(context.getContentResolver().openInputStream(part.getContentUri()), file);
                    } catch (FileNotFoundException e) {
                    }
                }
                Uri contentUri = Uri.fromFile(file);
                part.setContentUri(contentUri);
                rcsMsg = new RcsMessage(getRmsTypeFromMessagePartData(part), contentUri);
            } else if (part.isText()) {
                String atList = RcsGroupAtHelper.getAtListFromGlobalAndRemove(String.valueOf(message.getReceivedTimeStamp()));
                if (!TextUtils.isEmpty(atList)) {
                    rcsMsg = new RcsMessage(getRmsTypeFromMessagePartData(part), messageText, atList);
                } else {
                    /**
                     * juphoon chatbot reply 消息 mMsgConversationId 和 mExtra 非空
                     */
                    rcsMsg = new RcsMessage(getRmsTypeFromMessagePartData(part), messageText, part.mContributionId, part.mExtra, part.mTrafficType);                }
            }
            if (rcsMsg != null) {
                sendMessage(rcsMsg, threadId, rmsType, message, conversationId, rmsAddress);
            }
        }
    }

    // juphoon 发送消息
    private void sendMessage(RcsMessage rcsMsg, long threadId, int rmsType, MessageData message, String conversationId, String rmsAddress) {
        final Context context = Factory.get().getApplicationContext();
        final DatabaseWrapper db = DataModel.get().getDatabase();
        long laterTimestamp = rcsMsg.getDate();
        sLastSentMessageTimestamp = laterTimestamp;
        // Inform sync that message is being added at timestamp
        final SyncManager syncManager = DataModel.get().getSyncManager();
        syncManager.onNewMessageInserted(laterTimestamp);
        Uri messageUri = new RcsMessageSender(context, threadId, rmsType, rmsAddress, rcsMsg).sendSpreadMessage();
        if (messageUri != null && !TextUtils.isEmpty(messageUri.toString())) {
            BugleDatabaseOperations.deleteDirtyMessage(db, messageUri.toString());
            Intent intent = new Intent(RcsMessageSendService.ACTION_SEND_RCS_MESSAGE, null, Factory.get().getApplicationContext(), RcsMessageSendService.class);
            intent.putExtra(RcsMessageSendService.RCS_SEND_MESSAGE_FAILED_BUNDLE, actionParameters);
            // juphoon
    ...
    // juphoon
    private int getRmsTypeFromMessagePartData(MessagePartData part) {
        if (part.isAudio()) {
            return RmsDefine.Rms.RMS_MESSAGE_TYPE_VOICE;
        } else if (part.isVideo()) {
            return RmsDefine.Rms.RMS_MESSAGE_TYPE_VIDEO;
        } else if (part.isGeo()) {
            return RmsDefine.Rms.RMS_MESSAGE_TYPE_GEO;
        } else if (part.isVCard()) {
            return RmsDefine.Rms.RMS_MESSAGE_TYPE_VCARD;
        } else if (part.isImage()) {
            return RmsDefine.Rms.RMS_MESSAGE_TYPE_IMAGE;
        } else if (part.isFile()) {
            return RmsDefine.Rms.RMS_MESSAGE_TYPE_FILE;
        }
        return RmsDefine.Rms.RMS_MESSAGE_TYPE_TEXT;
    }
