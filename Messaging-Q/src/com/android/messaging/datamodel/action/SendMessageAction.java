...
public class SendMessageAction extends Action implements Parcelable {
    ...
    private boolean queueAction(final String messageId, final Action processingAction) {
        ...
        if (message != null && message.canSendMessage()) {
            ...
            final boolean isRms = (message.getProtocol() == MessageData.PROTOCOL_RMS); // juphoon
            ...
            // juphoon
            if (!updateMessageAndStatus(isRms, isSms, message, null /* messageUri */, false /*notify*/)) {
    ...
    protected Bundle doBackgroundWork() {
        ...
        if (isRms) { // rcs 消息不处理
            ...
            String recipient = actionParameters.getString(KEY_RECIPIENT);//juphoon
            ...
            /**
             * juphoon
             * 发短信如果是 chatbot 发送，则将发送地址替换为 chatbot 的 sms
             */
            RcsChatbotHelper.RcsChatbot chatbot = RcsChatbotHelper.getChatbotInfoByServiceId(recipient);
            if (chatbot != null && !TextUtils.isEmpty(chatbot.sms)) {
                recipient = chatbot.sms;
            }
            ...
            if (recipients.size() == 1) {
                /**
                 * juphoon
                 * 发彩信如果是 chatbot 发送，则将发送地址替换为 chatbot 的 sms
                 */
                RcsChatbotHelper.RcsChatbot chatbot = RcsChatbotHelper.getChatbotInfoByServiceId(recipients.get(0));
                if (chatbot != null && !TextUtils.isEmpty(chatbot.sms)) {
                    recipients.clear();
                    recipients.add(chatbot.sms);
                }
            }
            if (messageUri == null) {
        ...
        ProcessSentMessageAction.processMessageSentFastFailed(messageId, messageUri,
                updatedMessageUri, subId, false, isSms, status, rawStatus, resultCode);//juphoon
        return null;
    }
    ...
    protected Object processBackgroundFailure() {
        ...
        ProcessSentMessageAction.processResult(messageId, null /* updatedMessageUri */,
                ...
                isRms, isSms, this, subId, resultCode, httpStatusCode); // juphoon
     ...
     */
    public static boolean updateMessageAndStatus(final boolean isRms, final boolean isSms, final MessageData message, // juphoon
