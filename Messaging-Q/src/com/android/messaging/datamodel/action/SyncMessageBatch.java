...
class SyncMessageBatch {
    ...
    void updateLocalDatabase() {
        List<Long> rmsAttachSmsIds = RcsMmsUtils.getRmsAttachSmsIds();//juphoon
        ...
        try {
			...
			//juphoon
                if (count < mMessagesToDelete.size()) {
                    for (final LocalDatabaseMessage delSms : mMessagesToDelete) {
                        if (sms.getTimestampInMillis() == delSms.getTimestampInMillis()) {
                            count++;
                            break;
                        }
                    }
                }
                if (rmsAttachSmsIds.contains(sms.mRowId)) {
                    storeRms(db, RcsDatabaseMessages.RmsMessage.get(sms.mRowId));
                } else {
                    storeSms(db, sms);
                }
            }
            ...
            for (final LocalDatabaseMessage message : mMessagesToDelete) {
                if (LogUtil.isLoggable(TAG, LogUtil.VERBOSE)) {
                    LogUtil.v(TAG, "SyncMessageBatch: Deleted message " + message.getLocalId()
                            + " for SMS/MMS/RMS " + message.getUri() + " with timestamp "
                            + message.getTimestampInMillis()); // juphoon
                }
                /**
                 * juphoon，rms 表在 Messaging 中如果同步删除的消息是 rcs 消息，则要删除 rms 的相关记录
                 */
                if (message.getProtocol() == MessageData.PROTOCOL_RMS) {
                    MmsUtils.deleteMessage(Uri.parse(message.getUri()));
                }
            }
    ...
    /** juphoon
     * Store the RMS message into local database.
     *
     * @param rms
     */
    private void storeRms(final DatabaseWrapper db, final RcsDatabaseMessages.RmsMessage rms) {
        if (rms.mBody == null) {
            LogUtil.w(TAG, "SyncMessageBatch: RMS " + rms.mUri + " has no body; adding empty one");
            // try to fix it
            rms.mBody = "";
        }

        if (TextUtils.isEmpty(rms.mAddress)) {
            LogUtil.e(TAG, "SyncMessageBatch: RMS has no address; using unknown sender");
            // try to fix it
            rms.mAddress = ParticipantData.getUnknownSenderDestination();
        }

        // 如果是系统消息 rms.mAddress = system_msg
        if (rms.mMessageType == RmsDefine.Rms.RMS_MESSAGE_TYPE_SYSTEM) {
            rms.mAddress = RcsMmsUtils.RMS_SYSTEM_MSG;
        }
        // TODO : We need to also deal with messages in a failed/retry state
        final boolean isOutgoing = rms.mType != RmsDefine.Rms.MESSAGE_TYPE_INBOX;

        final String otherPhoneNumber = rms.mAddress;

        // A forced resync of all messages should still keep the archived states.
        // The database upgrade code notifies sync manager of this. We need to
        // honor the original customization to this conversation if created.
        final String conversationId = mCache.getOrCreateConversation(db, rms.mThreadId, rms.mSubId,
                DataModel.get().getSyncManager().getCustomizationForThread(rms.mThreadId));
        if (conversationId == null) {
            // Cannot create conversation for this message? This should not happen.
            LogUtil.e(TAG, "SyncMessageBatch: Failed to create conversation for SMS thread "
                    + rms.mThreadId);
        ...
        final ParticipantData self = ParticipantData.getSelfParticipant(rms.getSubId());
        final String selfId =
                BugleDatabaseOperations.getOrCreateParticipantInTransaction(db, self);
        final ParticipantData sender = isOutgoing ?
                self :
                ParticipantData.getFromRawPhoneBySimLocale(otherPhoneNumber, rms.getSubId());
        final String participantId = (isOutgoing ? selfId :
                BugleDatabaseOperations.getOrCreateParticipantInTransaction(db, sender));
        final int bugleStatus = bugleStatusForRms(isOutgoing, rms.mType, rms.mStatus);
        final MessageData message = RcsMmsUtils.createRmsMessage(rms, conversationId, participantId, selfId, bugleStatus);

        // Inserting rms content into messages table
        try {
            BugleDatabaseOperations.insertNewMessageInTransaction(db, message);
        } catch (SQLiteConstraintException e) {
            rethrowSQLiteConstraintExceptionWithDetails(e, db, rms.mUri, rms.mThreadId,
                    conversationId, selfId, participantId);
        }
    ...
    /** juphoon **/
    public static int bugleStatusForRms(final boolean isOutgoing, final int type,
            final int status) {
        int bugleStatus = MessageData.BUGLE_STATUS_UNKNOWN;
        switch (status) {
            case RmsDefine.Rms.STATUS_INIT:
                bugleStatus = isOutgoing ? MessageData.BUGLE_STATUS_OUTGOING_YET_TO_SEND
                        : MessageData.BUGLE_STATUS_INCOMING_YET_TO_MANUAL_DOWNLOAD;
                break;
            case RmsDefine.Rms.STATUS_PENDING:
                bugleStatus = isOutgoing ? MessageData.BUGLE_STATUS_OUTGOING_SENDING
                        : MessageData.BUGLE_STATUS_INCOMING_MANUAL_DOWNLOADING;
                break;
            case RmsDefine.Rms.STATUS_FAIL:
                bugleStatus = isOutgoing ? MessageData.BUGLE_STATUS_OUTGOING_FAILED
                        : MessageData.BUGLE_STATUS_INCOMING_DOWNLOAD_FAILED;
                break;
            case RmsDefine.Rms.STATUS_SUCC:
                bugleStatus = isOutgoing ? MessageData.BUGLE_STATUS_OUTGOING_COMPLETE
                        : MessageData.BUGLE_STATUS_INCOMING_COMPLETE;
                break;
            case RmsDefine.Rms.STATUS_RECEIVED:
                bugleStatus = MessageData.BUGLE_STATUS_OUTGOING_DELIVERED;
                break;
            case RmsDefine.Rms.STATUS_DISPLAYED:
                bugleStatus = MessageData.BUGLE_STATUS_OUTGOING_DISPLAYED;
                break;
