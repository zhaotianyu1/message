...
public class GetOrCreateConversationAction extends Action implements Parcelable {
    ...
    public static GetOrCreateConversationActionMonitor getOrCreateConversation(
        ...
        /**
         * Juphoon
         * 如果输入的号码是 Chatbot 则替换为 Chatbot 的 ParticipantData
         */
        if (participants.size() == 1) {
            ParticipantData participantData = participants.get(0);
            RcsChatbotHelper.RcsChatbot chatbot = RcsChatbotHelper.getChatbotInfoBySmsOrServiceNumber(RcsNumberUtils.formatPhoneNoCountryPrefix(participantData.getSendDestination()));
            if (chatbot != null) {
                participants.clear();
