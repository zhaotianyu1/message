...
public class DeleteConversationAction extends Action implements Parcelable {
    ...
    protected Bundle doBackgroundWork() throws DataModelException {
        ...
        if (!TextUtils.isEmpty(conversationId)) {
            ...
            if (threadId >= 0) {
                /** juphoon 中断删除单个会话时还在传输的文件 **/
                ArrayList<Long> threadIds = new ArrayList<>();
                threadIds.add(threadId);
                RcsMmsUtils.stopAllProgressingFileWhenDelete(Factory.get().getApplicationContext() , threadIds);
                final int count = MmsUtils.deleteThread(threadId, cutoffTimestamp);

                /**
                 * rms 表在 messaging 中则删除会话时要同时删除 rms 表的消息
                 */
                RcsMmsUtils.deleteRms(threadIds);
