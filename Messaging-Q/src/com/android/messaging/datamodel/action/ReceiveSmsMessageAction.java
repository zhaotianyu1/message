...
public class ReceiveSmsMessageAction extends Action implements Parcelable {
    ...
    protected Object executeAction() {
        ...
        if (TextUtils.isEmpty(address)) {
            ...
            /**
             * juphoon
             * chatbot 以 seviceId 作为会话标识，解决混排问题
             */
            RcsChatbotHelper.RcsChatbot chatbot = RcsChatbotHelper.getChatbotInfoBySmsOrServiceId(address);
            if (chatbot != null) {
                address = RcsChatbotHelper.formatServiceIdWithNoSip(chatbot.serviceId);
            }
        }
        ...
        if (!OsUtil.isSecondaryUser()) {
            ...
            String texts = BugleDatabaseOperations.getExistingConversations(db,message.getConversationId());
            LogUtil.i(TAG,"text---:"+texts);

            String name = BugleDatabaseOperations.getExistingName(db,message.getConversationId());
            LogUtil.i(TAG,"name---:"+name);
            Long time = BugleDatabaseOperations.getExistingTime(db,message.getConversationId());
           // String formattedTimestamp = Dates.getConversationTimeString(time).toString();
            LogUtil.i(TAG,"time---:"+time);

           // BugleDatabaseOperations.getConversation(db);
         //   api.getMessage(name,conversationId,texts,time);
