...
public class DeleteMessageAction extends Action implements Parcelable {
    ...
    protected Bundle doBackgroundWork() {
        ...
        if (!TextUtils.isEmpty(messageId)) {
            ...
            if (message != null) {
                /**
                 * juphoon 中断删除消息时还在传输的文件
                 */
                RcsTransProgressManager.deleteAndPauseMessage(messageId);
                ...
                if (messageUri != null) {
                    /**
                     * juphoon 如果是rcs消息且rms表不在TelephoneProvider里，则要删除该uri的消息，同时还要删除短信表的消息
                     */
                    if (TextUtils.equals(messageUri.getAuthority(), RmsDefine.RMS_AUTHORITY)) {
                        long id = RcsMmsUtils.getRmsAttachSmsId(messageUri);
                        if (id != -1) {
                            MmsUtils.deleteMessage(Telephony.Sms.CONTENT_URI.buildUpon().appendPath(String.valueOf(id)).build());
                        }
                    }
