...
public class MmsSmsUtils {
    ...
    public static final class Threads implements android.provider.Telephony.ThreadsColumns {
        ...
        public static long getOrCreateThreadId(
            ...
            /**
             * juphoon 创建底层 ThreadId 时判断 chatbot sms，如果是用 servieId 创建会话
             */
            boolean isChatbot = false;
            if (recipients.size() == 1) {
                String address = recipients.iterator().next();
                RcsChatbotHelper.RcsChatbot chatbot = RcsChatbotHelper.getChatbotInfoBySmsOrServiceId(address);
                if (chatbot != null) {
                    uriBuilder.appendQueryParameter("recipient", RcsChatbotHelper.formatServiceIdWithNoSip(chatbot.serviceId));
                    isChatbot = true;
                }
            }
            if (!isChatbot) {
